...
/*! \brief ldms_message_publish utility publisher_api implementation.
  This plugin generates a scratch file (in-memory) and asynchronously
  sends it to ldmsd by invoking the 'ldms_message_publish' utility
  available in ldms versions from 4.5. Use the ldmsd_stream_publish_plugin
  for ldms versions 4.4 and below.
 ...
 */
class ldms_message_publish_plugin : public publisher_api {
	enum state {
		ok,
		err
	};
...
public:
	/// \brief scratch directory for ldms_message_publish utility messages
	/// This directory must be globally writable and should be fast.
	/// Overridden with env("ADC_LDMS_MESSAGE_PUBLISH_PLUGIN_DIRECTORY").
	inline static const char* adc_ldms_message_publish_plugin_directory_default = "/dev/shm/adc";

	/// \brief full path to ldms_message_publish utility
	...
	inline static const char* adc_ldms_message_publish_plugin_auth_default = "munge";

	/// \brief port for message connections; ldmsd listeners must match.
	/// Overridden with env("ADC_LDMS_MESSAGE_PUBLISH_PLUGIN_PORT").
	inline static const char* adc_ldms_message_publish_plugin_port_default = "412";

	/// \brief host for message connections; ldmsd must be listening on the host.
	/// Overridden with env("ADC_LDMS_MESSAGE_PUBLISH_PLUGIN_HOST").
	inline static const char* adc_ldms_message_publish_plugin_host_default = "localhost";
	...
	/// Overridden with env("ADC_LDMS_MESSAGE_PUBLISH_PLUGIN_AFFINITY").
	inline static const char* adc_ldms_message_publish_plugin_affinity_default = "all";

	/// \brief ADC ldms-message plugin enable debug messages; (default "0": none)
	/// Overridden with env("ADC_LDMS_MESSAGE_PUBLISH_PLUGIN_DEBUG").
	inline static const char *adc_ldms_message_publish_plugin_debug_default = "0";
...
private:
	...
	{	{"DIRECTORY", adc_ldms_message_publish_plugin_directory_default},
		...
		{"DEBUG", adc_ldms_message_publish_plugin_debug_default},
		{"AFFINITY", adc_ldms_message_publish_plugin_affinity_default}
	};

	inline static const char *plugin_prefix = "ADC_LDMS_MESSAGE_PUBLISH_PLUGIN_";
	...
	int config(const string& dir, string_view shost, string_view sport, string_view sprog, string_view sauth, string_view schannel, const string& sdebug) {
		...
		if (debug > 0) {
			std::cout<< "ldms_message_publish plugin configured" <<std::endl;
		}
		return 0;
	}
	...
	const string get(const std::map< string, string >& m,
		...
		if (!ec) {
			return plugin_ldms_message_publish_config_defaults.at(field);
		} else {
			return string(ec);
		}
	}
	...
	// invoke ldms_message_publish with json in f, then delete f.
	// wrap this in a thread to make it non-blocking eventually.
	int ldms_message_publish_send(string& f)
	{
		string qcmd = "(" + prog + 
			" -t json "
			" -x sock "
...
public:
	ldms_message_publish_plugin() : vers("1.0.0") , tags({"none"}), debug(0),
		...
		if (mode != pi_pub_or_final)
			return 2;
		// write to tmpfile, ldms_message_publish, and then delete
		...
		if (out.good()) {
			...
			if (out.good()) {
				if (debug) {
					std::cout << "'ldms_message_publish' wrote" << std::endl;
				}
				out.close();
				ldms_message_publish_send(fname); // send also removes file
				return 0;
			} else {
	...
	const std::map< const std::string, const std::string> & get_option_defaults() {
		return plugin_ldms_message_publish_config_defaults;
	}
	...
	int initialize() {
		...
		if ( state == err ) {
			std::cout << "ldms_message_publish plugin initialize found pre-existing error" << std::endl;
			return 3;
		}
		std::error_code ec;
		...
		if (ec.value() != 0 && ec.value() != EEXIST ) {
			state = err;
			std::cout << "unable to create scratch directory for plugin 'ldms_message_publish'; "
		...
		if (mode == pi_pub_or_final) {
			...
			if (debug) {
				std::cout << "ldms_message_publish plugin finalize on non-running plugin" << std::endl;
			}
		}
	}
	...
	~ldms_message_publish_plugin() {
		if (debug) {
			std::cout << "Destructing ldms_message_publish_plugin" << std::endl;
		}
	}
};
